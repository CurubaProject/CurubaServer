...
var Request = {
	...
	LISTDEVICEREQUEST : 'SELECT Device.idDevice ID, Device.DeviceType Type, Device.DeviceState State, CustomInfo.Name Name FROM Device LEFT JOIN CustomInfo On CustomInfo.DeviceId = Device.idDevice WHERE ',
	DEVICEDETAIL : 'SELECT Device.idDevice ID, Device.DeviceType Type, Device.DeviceState State, CustomInfo.Name Name FROM Device Join CustomInfo On CustomInfo.DeviceId = Device.idDevice WHERE DeviceId = \'%s\'',
	SETDEVICENAME : 'UPDATE CustomInfo SET Name=\'%s\' WHERE DeviceId=%s',
	SETDEVICESTATE : 'UPDATE CustomInfo SET Name=\'%s\' WHERE DeviceId=%s',
	GETSTATISTICS : 'SELECT Statistic.Date, Statistic.Value FROM Statistic JOIN Device On Device.idDevice = Statistic.DeviceId JOIN TypeStatistic On TypeStatistic.idTypeStatistic = Statistic.TypeStatisticId WHERE Device.idDevice = \'%s\' AND TypeStatistic.Value = %s',
